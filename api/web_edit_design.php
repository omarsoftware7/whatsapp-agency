...
function log_edit_dispatch(string $message, array $context = []): void {
    $path = __DIR__ . '/../storage/edit_dispatch.log';
    $dir = dirname($path);
    if (!file_exists($dir)) {
        mkdir($dir, 0777, true);
    }
...
if ($user_status && (int)$user_status['is_active'] !== 1) {
    log_edit_dispatch('blocked_user', ['job_id' => $job_id, 'user_id' => $user_id]);
...
$job = $stmt->fetch();
if (!$job) {
    log_edit_dispatch('job_not_found', ['job_id' => $job_id]);
...
if (!$stmt->fetch()) {
    log_edit_dispatch('brand_not_found', ['job_id' => $job_id, 'user_id' => $user_id]);
    json_error('Brand not found', 404);
}

$primary_user = get_primary_web_user_for_client($db, (int)$job['client_id']);
if (!$primary_user) {
    log_edit_dispatch('no_primary_user', ['job_id' => $job_id, 'client_id' => (int)$job['client_id']]);
    json_error('No active user for this brand.', 403);
}
$primary_user = apply_credit_reset_if_needed($db, $primary_user);
if ((int)($primary_user['image_credits_remaining'] ?? 0) < 1) {
    log_edit_dispatch('no_credits', ['job_id' => $job_id, 'client_id' => (int)$job['client_id']]);
    json_error('Not enough image credits to edit a design.');
}

$disabled = array_map('trim', explode(',', (string)ini_get('disable_functions')));
if (!function_exists('exec') || in_array('exec', $disabled, true)) {
    log_edit_dispatch('exec_disabled', ['job_id' => $job_id]);
...
$stmt = $db->prepare("
    INSERT INTO web_design_edit_requests (job_id, client_id, user_edit)
    VALUES (?, ?, ?)
");
$stmt->execute([$job_id, $job['client_id'], $user_edit]);
...
log_activity($job['client_id'], $job_id, 'design_edit_requested_web', [
    'user_edit' => $user_edit
]);

$cmd = escapeshellcmd(PHP_BINARY) . ' ' .
    escapeshellarg(__DIR__ . '/tool_edit_design.php') . ' ' .
    escapeshellarg((string)$job_id) . ' ' .
    escapeshellarg($user_edit) . ' ' .
    escapeshellarg($image_url) . ' ' .
    escapeshellarg($edit_mode) .
    ' > /dev/null 2>&1 &';
log_edit_dispatch('dispatch', [
    'job_id' => $job_id,
    'cmd' => $cmd
]);
exec($cmd);
